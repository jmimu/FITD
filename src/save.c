...
void loadRotateStruc(rotateStruct* pRotateStruct, FILE* fHandle)
{
  ...
  if (fread(&pRotateStruct->oldAngle,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(pRotateStruct->newAngle) == 2);
  if (fread(&pRotateStruct->newAngle,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(pRotateStruct->param) == 2);
  if (fread(&pRotateStruct->param,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(pRotateStruct->timeOfRotate) == 2);
  if (fread(&pRotateStruct->timeOfRotate,2,1,fHandle)!=1) printf("Error reading save!\n");
}
...
int loadSave(int saveNumber)
{
  ...
  if (fread(&var28,4,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  if (fread(&currentDisplayedRoom,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(currentEtage) == 2);
  if (fread(&currentEtage,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(currentCamera) == 2);
  if (fread(&currentCamera,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(currentCameraTarget) == 2);
  if (fread(&currentCameraTarget,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(genVar9) == 2);
  if (fread(&genVar9,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(maxObjects) == 2);
  if (fread(&maxObjects,2,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  for(i=0;i<maxObjects;i++)
  {
    ...
    if (fread(&objectTable[i].ownerIdx,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].body) == 2);
    if (fread(&objectTable[i].body,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].flags) == 2);
    if (fread(&objectTable[i].flags,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].field_6) == 2);
    if (fread(&objectTable[i].field_6,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].foundBody) == 2);
    if (fread(&objectTable[i].foundBody,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].foundName) == 2);
    if (fread(&objectTable[i].foundName,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].flags2) == 2);
    if (fread(&objectTable[i].flags2,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].foundLife) == 2);
    if (fread(&objectTable[i].foundLife,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].x) == 2);
    if (fread(&objectTable[i].x,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].y) == 2);
    if (fread(&objectTable[i].y,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].z) == 2);
    if (fread(&objectTable[i].z,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].alpha) == 2);
    if (fread(&objectTable[i].alpha,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].beta) == 2);
    if (fread(&objectTable[i].beta,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].gamma) == 2);
    if (fread(&objectTable[i].gamma,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].stage) == 2);
    if (fread(&objectTable[i].stage,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].room) == 2);
    if (fread(&objectTable[i].room,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].lifeMode) == 2);
    if (fread(&objectTable[i].lifeMode,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].life) == 2);
    if (fread(&objectTable[i].life,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].field_24) == 2);
    if (fread(&objectTable[i].field_24,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].anim) == 2);
    if (fread(&objectTable[i].anim,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].frame) == 2);
    if (fread(&objectTable[i].frame,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].animType) == 2);
    if (fread(&objectTable[i].animType,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].animInfo) == 2);
    if (fread(&objectTable[i].animInfo,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].trackMode) == 2);
    if (fread(&objectTable[i].trackMode,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].trackNumber) == 2);
    if (fread(&objectTable[i].trackNumber,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(objectTable[i].positionInTrack) == 2);
    if (fread(&objectTable[i].positionInTrack,2,1,fHandle)!=1) printf("Error reading save!\n");
  }
  ...
  for(i=0;i<numCVars;i++)
  {
    ...
    if (fread(&CVars[i], 2, 1, fHandle)!=1) printf("Error reading save!\n");
  }

  ASSERT(sizeof(inHand) == 2);
  if (fread(&inHand,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(numObjInInventory) == 2);
  if (fread(&numObjInInventory,2,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  for(i=0;i<INVENTORY_SIZE;i++)
  {
    ...
    if (fread(&inventory[i],2,1,fHandle)!=1) printf("Error reading save!\n");
  }

  ASSERT(sizeof(statusScreenAllowed) == 2);
  if (fread(&statusScreenAllowed,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(giveUp) == 2);
  if (fread(&giveUp,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(lightVar1) == 2);
  if (fread(&lightVar1,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(shakingAmplitude) == 2);
  if (fread(&shakingAmplitude,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(shakeVar1) == 2);
  if (fread(&shakeVar1,2,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(timer) == 4);
  if (fread(&timer,4,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(timerFreeze1) == 4);
  if (fread(&timerFreeze1,4,1,fHandle)!=1) printf("Error reading save!\n");

  ASSERT(sizeof(currentMusic) == 2);
  if (fread(&currentMusic,2,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  if (fread(&offsetToVars,4,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  if (fread(&tempVarSize,2,1,fHandle)!=1) printf("Error reading save!\n");
  varSize = tempVarSize;

  if (fread(vars,varSize,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  if (fread(&offsetToActors,4,1,fHandle)!=1) printf("Error reading save!\n");
  ...
  for(i=0;i<NUM_MAX_ACTOR;i++)
  {
    ...
    if (fread(&actorTable[i].field_0,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].bodyNum) == 2);
    if (fread(&actorTable[i].bodyNum,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].flags) == 2);
    if (fread(&actorTable[i].flags,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].dynFlags) == 2);
    if (fread(&actorTable[i].dynFlags,2,1,fHandle)!=1) printf("Error reading save!\n");

//    ASSERT(sizeof(actorTable[i].zv.ZVX1) == 2);
    if (fread(&actorTable[i].zv.ZVX1,2,1,fHandle)!=1) printf("Error reading save!\n");
    actorTable[i].zv.ZVX1 = (short int)actorTable[i].zv.ZVX1;

//    ASSERT(sizeof(actorTable[i].zv.ZVX2) == 2);
    if (fread(&actorTable[i].zv.ZVX2,2,1,fHandle)!=1) printf("Error reading save!\n");
    actorTable[i].zv.ZVX2 = (short int)actorTable[i].zv.ZVX2;

//    ASSERT(sizeof(actorTable[i].zv.ZVY1) == 2);
    if (fread(&actorTable[i].zv.ZVY1,2,1,fHandle)!=1) printf("Error reading save!\n");
    actorTable[i].zv.ZVY1 = (short int)actorTable[i].zv.ZVY1;

//    ASSERT(sizeof(actorTable[i].zv.ZVY2) == 2);
    if (fread(&actorTable[i].zv.ZVY2,2,1,fHandle)!=1) printf("Error reading save!\n");
    actorTable[i].zv.ZVY2 = (short int)actorTable[i].zv.ZVY2;

//    ASSERT(sizeof(actorTable[i].zv.ZVZ1) == 2);
    if (fread(&actorTable[i].zv.ZVZ1,2,1,fHandle)!=1) printf("Error reading save!\n");
    actorTable[i].zv.ZVZ1 = (short int)actorTable[i].zv.ZVZ1;

//    ASSERT(sizeof(actorTable[i].zv.ZVZ2) == 2);
    if (fread(&actorTable[i].zv.ZVZ2,2,1,fHandle)!=1) printf("Error reading save!\n");
    actorTable[i].zv.ZVZ2 = (short int)actorTable[i].zv.ZVZ2;

    ASSERT(sizeof(actorTable[i].field_14) == 2);
    if (fread(&actorTable[i].field_14,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_16) == 2);
    if (fread(&actorTable[i].field_16,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_18) == 2);
    if (fread(&actorTable[i].field_18,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_1A) == 2);
    if (fread(&actorTable[i].field_1A,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].roomX) == 2);
    if (fread(&actorTable[i].roomX,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].roomY) == 2);
    if (fread(&actorTable[i].roomY,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].roomZ) == 2);
    if (fread(&actorTable[i].roomZ,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].worldX) == 2);
    if (fread(&actorTable[i].worldX,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].worldY) == 2);
    if (fread(&actorTable[i].worldY,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].worldZ) == 2);
    if (fread(&actorTable[i].worldZ,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].alpha) == 2);
    if (fread(&actorTable[i].alpha,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].beta) == 2);
    if (fread(&actorTable[i].beta,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].gamma) == 2);
    if (fread(&actorTable[i].gamma,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].room) == 2);
    if (fread(&actorTable[i].room,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].stage) == 2);
    if (fread(&actorTable[i].stage,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].lifeMode) == 2);
    if (fread(&actorTable[i].lifeMode,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].life) == 2);
    if (fread(&actorTable[i].life,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].CHRONO) == 4);
    if (fread(&actorTable[i].CHRONO,4,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].ROOM_CHRONO) == 4);
    if (fread(&actorTable[i].ROOM_CHRONO,4,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].ANIM) == 2);
    if (fread(&actorTable[i].ANIM,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_40) == 2);
    if (fread(&actorTable[i].field_40,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_42) == 2);
    if (fread(&actorTable[i].field_42,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_44) == 2);
    if (fread(&actorTable[i].field_44,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_46) == 2);
    if (fread(&actorTable[i].field_46,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_48) == 2);
    if (fread(&actorTable[i].field_48,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].FRAME) == 2);
    if (fread(&actorTable[i].FRAME,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_4C) == 2);
    if (fread(&actorTable[i].field_4C,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].END_FRAME) == 2);
    if (fread(&actorTable[i].END_FRAME,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].END_ANIM) == 2);
    if (fread(&actorTable[i].END_ANIM,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].trackMode) == 2);
    if (fread(&actorTable[i].trackMode,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].trackNumber) == 2);
    if (fread(&actorTable[i].trackNumber,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].MARK) == 2);
    if (fread(&actorTable[i].MARK,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].positionInTrack) == 2);
    if (fread(&actorTable[i].positionInTrack,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].modX) == 2);
    if (fread(&actorTable[i].modX,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].modY) == 2);
    if (fread(&actorTable[i].modY,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].modZ) == 2); // 45
    if (fread(&actorTable[i].modZ,2,1,fHandle)!=1) printf("Error reading save!\n");

    loadRotateStruc(&actorTable[i].field_60,fHandle);

    ASSERT(sizeof(actorTable[i].falling) == 2);
    if (fread(&actorTable[i].falling,2,1,fHandle)!=1) printf("Error reading save!\n");

    loadRotateStruc(&actorTable[i].rotate,fHandle);

    ASSERT(sizeof(actorTable[i].field_72) == 2); 
    if (fread(&actorTable[i].field_72,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].speed) == 2);
    if (fread(&actorTable[i].speed,2,1,fHandle)!=1) printf("Error reading save!\n");

    loadRotateStruc(&actorTable[i].speedChange,fHandle);

    ASSERT(sizeof(actorTable[i].COL[0]) == 2);
    if (fread(&actorTable[i].COL[0],2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].COL[1]) == 2);
    if (fread(&actorTable[i].COL[1],2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].COL[2]) == 2);
    if (fread(&actorTable[i].COL[2],2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].COL_BY) == 2);
    if (fread(&actorTable[i].COL_BY,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].HARD_DEC) == 2);
    if (fread(&actorTable[i].HARD_DEC,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].HARD_COL) == 2);
    if (fread(&actorTable[i].HARD_COL,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].HIT) == 2);
    if (fread(&actorTable[i].HIT,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].HIT_BY) == 2);
    if (fread(&actorTable[i].HIT_BY,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].animActionType) == 2);
    if (fread(&actorTable[i].animActionType,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].animActionANIM) == 2);
    if (fread(&actorTable[i].animActionANIM,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].animActionFRAME) == 2);
    if (fread(&actorTable[i].animActionFRAME,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].animActionParam) == 2);
    if (fread(&actorTable[i].animActionParam,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].hitForce) == 2);
    if (fread(&actorTable[i].hitForce,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].field_98) == 2);
    if (fread(&actorTable[i].field_98,2,1,fHandle)!=1) printf("Error reading save!\n");

    ASSERT(sizeof(actorTable[i].hotPoint.x) == 2);
    if (fread(&actorTable[i].hotPoint.x,2,1,fHandle)!=1) printf("Error reading save!\n");
